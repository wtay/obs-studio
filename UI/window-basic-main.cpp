...
void OBSBasic::OBSInit()
{
	...
	vcamEnabled = (obs_get_output_flags(DEFAULT_VCAM_ID) &
...
{
	if (vcamConfigDialog.isNull()) {
		vcamConfigDialog = new OBSBasicVCamConfig(vcamConfig, this);
		connect(vcamConfigDialog, &OBSBasicVCamConfig::Accepted, this,
			&OBSBasic::UpdateVirtualCamConfig);
	} else
		vcamConfigDialog->SetConfig(vcamConfig);

	vcamConfigDialog->exec();
}
