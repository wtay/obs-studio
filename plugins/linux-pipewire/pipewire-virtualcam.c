...
#include "pipewire-stream-video-output.h"
...
static bool virtualcam_start(void *data)
{
	...
	obs_pwvc->obs_pw = obs_pipewire_create(-1, NULL, obs_pwvc->obs_pw);
	if (!obs_pwvc->obs_pw) {
		blog(LOG_WARNING, "Failed to create core");
		return false;
	}

	obs_pwvc->obs_pw_stream =
		obs_pipewire_create_stream_video_output(obs_pwvc->output);

	if (!obs_pipewire_connect_stream(
		    obs_pwvc->obs_pw, obs_pwvc->obs_pw_stream, PW_ID_ANY,
		    "OBS Studio",
		    pw_properties_new(PW_KEY_NODE_DESCRIPTION,
				      "OBS Virtual Camera (PipeWire)", PW_KEY_MEDIA_CLASS,
