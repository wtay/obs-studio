...
#include "pipewire.h"
#include "pipewire-stream-video-async.h"
...
static void stream_camera(struct camera_portal_source *camera_source)
{
	...
	camera_source->obs_pw_stream =
		obs_pipewire_create_stream_video_async(camera_source->source);
	obs_pipewire_connect_stream(
		connection->obs_pw, camera_source->obs_pw_stream, device->id,
		"OBS PipeWire Camera",
...
static void camera_format_list(struct camera_device *dev, obs_property_t *prop)
{
	...
	spa_list_for_each(p, &dev->param_list, link) {
		...
		if (!lookup_format_info_from_spa_format(format, &info))
			continue;

		if (info.video_format == last_format)
			continue;

		last_format = info.video_format;

		obs_property_list_add_int(prop, info.pretty_name, format);
	}
}
...
static void camera_update_controls(struct camera_device *dev, obs_properties_t *props,
		obs_data_t *settings)
{
	struct param *p;
	spa_list_for_each(p, &dev->param_list, link) {
			    ...
			    obs_property_t *property, obs_data_t *settings)
{
	...
	camera_format_list(device, prop);
	camera_update_controls(device, ctrl_props_new, settings);
 ...
 */
static bool format_selected(void *data, obs_properties_t *props,
 ...
 */
static bool resolution_selected(void *data, obs_properties_t *props,
				obs_property_t *p, obs_data_t *settings)
{
	struct camera_portal_source *camera_source = data;
	struct camera_device *device;
...
static void node_info(void *data, const struct pw_node_info *info)
{
	...
	if (info->change_mask & PW_NODE_CHANGE_MASK_PARAMS) {
		for (i = 0; i < info->n_params; i++) {
			...
			res = pw_node_enum_params((struct pw_node*)device->proxy,
	...
	if (changed) {
		...
		device->pending_sync = pw_proxy_sync(device->proxy,
				device->pending_sync);
	}
}
...
static void on_done_proxy_cb(void *data, int seq)
{
	...
	if (device->info != NULL && device->pending_sync == seq)
		object_update_params(&device->param_list, &device->pending_list,
			device->info->n_params, device->info->params);
}
				  ...
				  const struct spa_dict *props)
{
	...
	blog(LOG_INFO, "[pipewire-camera] Found device %s", device_id);
	...
	pw_node_add_listener(device->node, &device->node_listener,
			      &node_events, device);
